...
import { initCheckboxes, toggleChildCheckbox, toggleParentCheckbox } from "store/slices/filterBarSlice"
...
const CategoryCheckBoxesContainer = () => {
    ...
    const parentOnClick = (parentName) => dispatch(toggleParentCheckbox({
            parentName,
            keys: CATEGORY_DROPDOWN_KEYS
        }))

    const childOnClick = (parentName, childName) => dispatch(toggleChildCheckbox({
        parentName, 
        childName,
        keys: CATEGORY_DROPDOWN_KEYS
    }))
